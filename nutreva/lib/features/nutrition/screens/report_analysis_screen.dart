...
class _ReportAnalysisScreenState extends State<ReportAnalysisScreen> {
  ...
  Future<void> _analyzeReport() async {
    ...
    try {
      ...
      final userResponse = await Supabase.instance.client.from('profiles').select().eq('id', authUser?.id ?? '').single();
      final contextStr = "Patient: ${userResponse['age']} year old ${userResponse['gender']}, Role: ${userResponse['role']}.";
      ...
      if (response.statusCode == 200) {
        ...
        // 5. Save to Supabase (medical_reports table)
        await Supabase.instance.client.from('medical_reports').insert({
          'user_id': authUser!.id,
          'ai_summary': analysis,
          'next_step': 'Review with professional',
        ...
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Report Secured on Blockchain & Database!'),
