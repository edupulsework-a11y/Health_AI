...
import 'package:nutreva/services/health_service.dart';
import 'package:nutreva/services/sensor_service.dart';
...
class _FoodScanScreenState extends State<FoodScanScreen> {
  ...
  String? _selectedMode; // 'cooked', 'label', or 'freshness'
  ...
  Future<void> _analyzeFreshness() async {
    // Just a wrapper to trigger analysis with freshness mode
    await _analyzeImage();
  }
  ...
  Future<void> _analyzeImage() async {
    ...
    try {
      ...
      if (user != null) {
         userData = await Supabase.instance.client.from('profiles').select().eq('id', user.id).maybeSingle();
      }

      final String userContext = userData != null 
          ? "User identifying as ${userData['gender']}, aged ${userData['age']}. Role: ${userData['role']}."
          : "Standard nutritional context.";

      String prompt = "";
      if (_selectedMode == 'cooked') {
        prompt = "Analyze this COOKED MEAL image. Context: $userContext Quantity: $_mealQuantity. \n"
            "CRITICAL: Return ONLY a raw JSON object. NO conversational text, NO intro, NO outro.\n"
            "REQUIRED JSON FORMAT: {'items': [{'item_name': 'string', 'total_calories': int, 'total_protein': int, 'total_carbs': int, 'total_fats': int}], 'health_score': int (0-100), 'verdict': 'EAT/AVOID', 'reasoning': 'short explanation', 'advice': 'short tip', 'maternity_suitability': 'SAFE/CAUTION'}";
      } else if (_selectedMode == 'label') {
        prompt = "Analyze this FOOD LABEL image. Context: $userContext.\n"
            "CRITICAL: Return ONLY a raw JSON object. NO conversational text, NO intro, NO outro.\n"
        ...
        // Freshness Logic with Sensor Data
        final sensorData = await _sensorService.getCurrentSensorData();
        final gas = sensorData['gas_ppm'] ?? 0;
        final temp = sensorData['temperature'] ?? 25;
        final humidity = sensorData['humidity'] ?? 50;
        final iotStatus = _sensorService.getFoodStatus(gas);

        prompt = "Analyze this FRUIT/VEGETABLE for FRESHNESS. Context: $userContext.\n"
            "IOT SENSOR DATA: Gas Level: $gas ppm, Temperature: $temp°C, Humidity: $humidity%. \n"
            "IOT ANALYSIS: $iotStatus. \n"
            "TASKS: 1. Identify the fruit. 2. Compare visual appearance with IOT data. 3. Provide final safety verdict.\n"
            "REQUIRED JSON FORMAT: {'items': [{'item_name': 'Freshness Check', 'total_calories': 0, 'total_protein': 0, 'total_carbs': 0, 'total_fats': 0}], 'health_score': int, 'verdict': 'EAT/THROW', 'reasoning': 'Explain based on visual and sensor: $gas ppm gas', 'advice': 'Storage tip'}";
      }
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  children: [
                    ...
                    _buildGlassCard(
                      "FRUIT FRESHNESS SCAN",
                      "Check ripeness via IOT gas sensors",
                      Icons.eco_rounded,
                      () => _showSourceSelection('freshness'),
                    ),
                    const SizedBox(height: 20),
                    _buildGlassCard(
                      "SCAN COOKED MEAL",
